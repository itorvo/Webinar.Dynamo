...
using Webinar.Dynamo.Repository.Exceptions;
using Webinar.Dynamo.Repository.Model;
...
namespace Webinar.Dynamo.Repository.Extensions
{
    public static class BaseExtensions
    {
            ...
            foreach (var property in properties)
            {
                ...
                else if (propertyType.GetInterfaces().Contains(typeof(IEnumerable)))
                {
                    if (propertyType.IsGenericType && propertyType.GetGenericTypeDefinition() == typeof(IDictionary<,>))
                    {
                        ...
                        IDictionary dictionary = (IDictionary)property.GetValue(entity);
        ...
        {
            return Attribute.IsDefined(propertyInfo, typeof(DynamoDBHashKeyAttribute)) || Attribute.IsDefined(propertyInfo, typeof(DynamoDBRangeKeyAttribute));
        }
